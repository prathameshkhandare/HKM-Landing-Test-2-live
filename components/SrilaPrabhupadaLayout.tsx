...
export default function SrilaPrabhupadaLayout({ title, subtitle, heroImage, children, customHeader }: SrilaPrabhupadaLayoutProps) {
    ...
    return (
        <main ref={containerRef} className="min-h-screen bg-[#FFF9F0] font-sans text-gray-900 selection:bg-[#FFB81C] selection:text-white relative is-pattern-bg">
            ...
            {/* Standard Header or Custom Header */}
                    ...
                    <div className="lg:col-span-3 lg:order-1 order-2">
                        <div className="sticky top-32 space-y-8">
                            <div className="bg-white p-6 rounded-2xl shadow-xl border border-gray-100 hidden lg:block">
                                <h3 className="text-[#2D0A0A] font-bold uppercase tracking-widest text-xs mb-6 flex items-center gap-2 font-serif">
                                    <BookOpen size={14} className="text-[#FFB81C]" /> Srila Prabhupada
                                </h3>
                                    ...
                                    {navItems.map((item) => {
                                        ...
                                        return (
                                            <Link 
                                                key={item.name} 
                                                href={item.href}
                                                className={`group flex items-center justify-between px-4 py-3 rounded-lg text-sm transition-all duration-300 ${isActive ? 'bg-[#2D0A0A] text-white font-medium shadow-md' : 'text-gray-600 hover:bg-[#FFF9F0] hover:text-[#2D0A0A]'}`}
                                            >
                                                <div className="flex items-center gap-3">
                                                    <item.icon size={16} className={isActive ? "text-[#FFB81C]" : "text-gray-400 group-hover:text-[#2D0A0A]"} />
                                                    {item.name}
                                                </div>
                                                {isActive && <ChevronRight size={14} className="text-[#FFB81C]" />}
                                            </Link>
                                        )
                                    })}
                                </nav>
                            ...
                            {/* Decorative Quote */}
                            <div className="p-6 bg-[#FFB81C]/10 rounded-2xl border border-[#FFB81C]/20 hidden lg:block">
                                <p className="text-[#2D0A0A] font-serif italic text-sm leading-relaxed">
                                    "Books are the basis. Preaching is the essence. Utility is the principle. Purity is the force."
                                </p>
                                <span className="block text-right text-xs font-bold text-[#2D0A0A]/60 mt-2">— Srila Prabhupada</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-9 lg:order-2 order-1">
                        ...
                        >
                            <div className="prose prose-xl prose-stone max-w-none 
                                ...
                                prose-li:text-gray-600 prose-li:marker:text-[#FFB81C]
                            ">
                                {children}
                            </div>
                        </motion.div>
                           ...
                           <div className="mt-12 lg:hidden">
                            ...
                            <div className="grid gap-3">
                                {navItems.map((item) => (
                                    ...
                                    >
                                        <item.icon size={18} className="text-[#2D0A0A]" />
                                        {item.name}
                                    </Link>
                                ))}
                            </div>
